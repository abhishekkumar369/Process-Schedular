...
function addRow(){
    ...
    if(AT=="" ||BT==""){
        ...
        inpPr.disabled = document.getElementById("PR").checked ? false : true;
...
function calAverage(p){
    ...
    avgrt=avgrt.toPrecision(3);
    avgtat=avgtat.toPrecision(3);
    avgct=avgct.toPrecision(3);
    avgwt=avgwt.toPrecision(3);

    document.getElementById('averagebox1').value=avgrt;
    document.getElementById('averagebox2').value=avgtat;
    document.getElementById('averagebox3').value=avgct;
    document.getElementById('averagebox4').value=avgwt;
}
...
function calculate(){
    ...
    //FCFS
        ...
        let time = 0;
        let processes = table.rows.length - 2; // Assuming table includes header rows
    
        for (let i = 0; i < processes; i++) {
            let pr = document.getElementById("processr");
            let tr = document.getElementById("timer");
            let at = Number(table.rows[2 + i].cells[1].innerHTML); // Arrival Time
            let bt = Number(table.rows[2 + i].cells[2].innerHTML); // Burst Time
    
            if (at > time) {
                // If there is idle time before the process arrives, animate idle time
                animator(pr, tr, 0, at - time);
                time = at; // Update time to the arrival time of the process
            }
    
            // Calculate Response Time
            let rt = time - at;
            table.rows[2 + i].cells[4].innerHTML = "" + rt;
    
            // Animate the process execution
            animator(pr, tr, i + 1, bt);
    
            // Update time to the completion time of the current process
            time += bt;
    
            let ct = time; // Completion Time
            let tat = ct - at; // Turnaround Time
            table.rows[2 + i].cells[6].innerHTML = "" + ct;
            table.rows[2 + i].cells[5].innerHTML = "" + tat;
    
            // Calculate Waiting Time
            let wt = rt; // Waiting Time is the same as Response Time in FCFS
            table.rows[2 + i].cells[7].innerHTML = "" + wt;
        }
    
        calAverage(processes);
        create_Graph(); // You need to implement the Gantt chart creation based on the time calculations
    }
    ...
    */
    ...
    //Priority
        ...
        while(queue.length!=0){
            ...
            if (f == 0) {
                let responseTime = time - Number(table.rows[1 + index].cells[1].innerHTML);
                table.rows[1 + index].cells[4].innerHTML = responseTime;
